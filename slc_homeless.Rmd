...
combined <- homeless %>%
  ...
  select(id, date, cases_avg7, has_income, district, 
...
mymap <- get_map(location = bb, zoom = 12,
...
```{r scale-data}
X <- model.matrix(~ -1 + district + has_income + density + logincome + cases_avg7,
                  data = combined)
vars_scaled <- scale(X[,9:11])
X[,9:11] <- vars_scaled
...
logprior <- function(beta) {
  ...
  sum( dnorm(beta, mu_0, 1/tau2_0, log = TRUE) )
}
...
```{r sample-slice}
doit <- function(ndraws = 10000, width = rep(1, 11)) {
  n_evals <- 0
  f <- function(pars) {
    n_evals <<- n_evals + 1
    logpost_adj(pars)
  }
  draws <- matrix(0, nrow = ndraws, ncol = 11)
  ...
  for (i in 1:ndraws) {
    for (j in 1:11) {
      ...
      l_prop <- current; l_prop[j] <- l
      u <- l + width[j]
      u_prop <- current; u_prop[j] <- u
...
ndraws <- 1e3
test <- doit(ndraws = ndraws, width = c(0.5, 11))

ess <- floor( c(rep(ndraws, 6), 
                as.numeric(coda::effectiveSize(test$draws[,7:8]))) )
names(ess) <- c('Tau1', 'Tau2', 'Tau3', 'Tau4', 'Tau5', 'Tau6',
                'a', 'b')
...
```{r stan-data-model2}
# Need
# vector of sample sizes for all 7 districts
# matrix (7 x maxN) of days open (RESPONSE VARIABLE)
# matrix (7 x maxN) of 7-day rolling average of COVID cases
...
y <- homeless %>%
  ...
  ungroup() %>%
  select(obs_num, district, days_open) %>%
  pivot_wider(id_cols = c(obs_num, district),
              names_from = obs_num,
              values_from = days_open) %>%
  select(-district) %>%
  as.matrix()
y[is.na(y)] <- 1e3
...
x <- homeless %>%
  ...
  mutate(obs_num = row_number()) %>%
  ungroup() %>%
  select(obs_num, district, cases_avg7) %>%
  pivot_wider(id_cols = c(obs_num, district),
              names_from = obs_num,
              values_from = cases_avg7) %>%
  select(-district) %>%
  as.matrix()
x[is.na(x)] <- 1e3
...
# Covid cases
x <- model.matrix(~ -1 + district + cases_avg7,
             data = homeless %>%
               left_join(covid, by = 'date'))
...
```{r stan-fit}
# Fit Stan model
m <- stan_model(model_code = readLines("model3.stan"))
...
```{r}
# Fit Stan model
m <- stan_model(model_code = readLines("model4.stan"))
data <- list(N = N,
             y = y, 
             X = X,
             mu_0 = mu_0,
